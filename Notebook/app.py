...
# Cấu hình
model_path = r"D:\Folder_HocTap\Đồ án tốt nghiệp\Code\Thesis_FineTune_MoE_ChatBotDental\Model\Model_demo_40K\DentalGPT_demo_40k"
max_seq_length = 512

# Kiểm tra CUDA và di chuyển mô hình lên GPU nếu có
device = "cuda" if torch.cuda.is_available() else "cpu"

# Load tokenizer
tokenizer = AutoTokenizer.from_pretrained(model_path)

# Load model và di chuyển mô hình lên GPU nếu có, sử dụng device_map để phân phối mô hình
model = AutoModelForCausalLM.from_pretrained(
    model_path,
    torch_dtype=torch.float16,
    device_map="auto",  # Tự động phân phối mô hình giữa GPU và CPU
...
def chatbot_stream(user_input, cot_goal, cot_reasoning, cot_justification):
    ...
    inputs = tokenizer(prompt, return_tensors="pt").to(device)  # Di chuyển input lên GPU
    ...
    generation_kwargs = dict(
        **inputs,
        ...
        device=device  # Thêm device="cuda"
    )
